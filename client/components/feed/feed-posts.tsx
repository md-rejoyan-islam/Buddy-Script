...
import { useLikePost, useDeletePost, useUpdateVisibility } from "@/hooks/use-post-mutations";
  ...
  return (
    <div className="flex gap-2.5 mb-3">
      <div className="shrink-0">
        ...
        <img src={authorImage} alt={authorName} className="w-7 h-7 rounded-full object-cover" />
      </div>
      <div className="flex-1 min-w-0">
        <div className="flex items-start gap-1">
          <div className="flex-1 bg-(--comment-bg) rounded-xl p-2.5">
            <h4 className="font-medium text-xs text-(--color6) mb-0.5">{authorName}</h4>
            {isEditing ? (
              <div>
                <input
                  type="text"
                ...
                <div className="flex items-center gap-2 mt-1">
                  <button onClick={handleEditSubmit} className="text-[10px] text-(--color5) bg-transparent border-none cursor-pointer">Save</button>
                  <button onClick={() => { setIsEditing(false); setEditText(reply.content); }} className="text-[10px] text-(--color7) bg-transparent border-none cursor-pointer">Cancel</button>
                </div>
              </div>
            ) : (
          ...
          {isOwner && !isEditing && (
            <div className="relative shrink-0" ref={menuRef}>
              ...
              >
                <svg width="12" height="4" viewBox="0 0 14 4" fill="currentColor">
        ...
        <div className="flex items-center gap-3 mt-1 px-1">
          <div className="relative group">
            <div className="absolute bottom-full left-0 mb-1 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-50">
              <div className="bg-(--bg2) rounded-full shadow-lg border border-(--bcolor1) px-1.5 py-1 flex items-center gap-0.5">
                {Object.entries(REACTIONS).map(([key, emoji]) => (
                  <button
                    key={key}
                    onClick={() => likeReply.mutate({ replyId: reply.id, reaction: key })}
                    title={key}
            ...
            <button
              onClick={() => likeReply.mutate({ replyId: reply.id, reaction: currentReaction })}
            ...
            >
              ...
              {isLiked ? currentReaction.charAt(0).toUpperCase() + currentReaction.slice(1) : "Like"}
              {reply._count.likes > 0 && ` · ${reply._count.likes}`}
            </button>
          </div>
          <span className="text-xs text-(--color7)">{formatDistanceToNow(reply.createdAt)}</span>
        </div>
      </div>
    </div>
  );
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-start gap-1">
          <div className="flex-1 bg-(--comment-bg) rounded-xl p-3">
            <h4 className="font-medium text-sm text-(--color6) mb-1">{authorName}</h4>
            {isEditing ? (
              <div>
                <input
                  type="text"
                ...
                />
                <div className="flex items-center gap-2 mt-1">
                  <button onClick={handleEditSubmit} className="text-xs text-(--color5) bg-transparent border-none cursor-pointer">Save</button>
                  <button onClick={() => { setIsEditing(false); setEditText(comment.content); }} className="text-xs text-(--color7) bg-transparent border-none cursor-pointer">Cancel</button>
                </div>
              </div>
            ) : (
              <p className="text-sm text-(--color7) whitespace-pre-line">{comment.content}</p>
            )}
          </div>
          {isOwner && !isEditing && (
            <div className="relative shrink-0" ref={menuRef}>
        ...
        <div className="flex items-center gap-3 mt-1 px-1">
          <div className="relative group">
            <div className="absolute bottom-full left-0 mb-1 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-50">
              <div className="bg-(--bg2) rounded-full shadow-lg border border-(--bcolor1) px-1.5 py-1 flex items-center gap-0.5">
                {Object.entries(REACTIONS).map(([key, emoji]) => (
                  <button
                    key={key}
                    onClick={() => likeComment.mutate({ commentId: comment.id, reaction: key })}
                    title={key}
                    className="text-base hover:scale-125 transition-transform duration-150 bg-transparent border-none cursor-pointer p-0.5"
                  >
                    {emoji}
            ...
            <button
              onClick={() => likeComment.mutate({ commentId: comment.id, reaction: currentReaction })}
              className={`text-xs border-none bg-transparent cursor-pointer hover:underline ${isLiked ? "text-(--color5) font-medium" : "text-(--color7)"}`}
            >
              {isLiked ? `${REACTIONS[currentReaction] || "👍"} ` : ""}
              {isLiked ? currentReaction.charAt(0).toUpperCase() + currentReaction.slice(1) : "Like"}
          ...
          <button
            onClick={() => {
              ...
              if (!showReplies && comment._count.replies > 0) setShowReplies(true);
            }}
            className="text-xs text-(--color7) border-none bg-transparent cursor-pointer hover:underline"
          >
            Reply
          </button>
          <span className="text-xs text-(--color7)">{formatDistanceToNow(comment.createdAt)}</span>
        </div>
        ...
        {showReplies && (
          <div className="mt-3 pl-2">
            ...
            {replies?.map((reply) => (
              <ReplyItem key={reply.id} reply={reply} commentId={comment.id} postId={postId} currentUserId={currentUserId} />
            ))}
          </div>
        )}
          ...
          <form onSubmit={handleReplySubmit} className="mt-2">
            <div className="bg-(--comment-bg) rounded-[18px] py-1 px-2.5">
              <div className="flex items-center">
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img src="/images/txt_img.png" alt="You" className="w-5 h-5 rounded-full object-cover shrink-0" />
                <input
                  type="text"
  ...
  return (
    <div className="px-6 pt-4 pb-2">
      ...
      {comments?.map((comment) => (
        <CommentItem key={comment.id} comment={comment} postId={postId} currentUserId={currentUser?.id} />
      ))}

      {comments && comments.length === 0 && (
        <p className="text-xs text-(--color7) text-center py-2">No comments yet</p>
      )}
    </div>
  );
}
...
export function FeedPostCard({ post, currentUserId }: { post: Post; currentUserId?: string }) {
    ...
    <div className="bg-(--bg2) rounded-md pb-6 pt-6 mb-4">
      <div className="px-6">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center">
            <div className="mr-3">
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img src={authorImage} alt={authorName} className="w-11 h-11 rounded-full object-cover" />
            </div>
            <div>
              <h4 className="font-normal text-base leading-[1.1] text-(--color6) cursor-pointer hover:underline">{authorName}</h4>
              <p className="font-normal text-sm leading-[1.2] text-(--color)">
                {timeAgo} . <span>{post.visibility === "PUBLIC" ? "Public" : "Private"}</span>
              </p>
            </div>
          </div>
          {/* 3-dot menu — only for post owner */}
          {isOwner && <div className="relative" ref={menuRef}>
            <button
              onClick={() => setShowMenu(!showMenu)}
        ...
        {post.image && (
          <div className="mb-6">
            {/* eslint-disable-next-line @next/next/no-img-element */}
            <img src={post.image} alt="Post" className="w-full h-auto rounded-md object-cover" />
          </div>
        )}
      </div>
      ...
      <div className="flex items-center bg-(--reaction-bg) rounded-md p-2 mx-6">
        <div className="flex-1 relative group mr-1">
          ...
          <div className="absolute bottom-full left-0 mb-2 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-50">
            <div className="bg-(--bg2) rounded-full shadow-lg border border-(--bcolor1) px-2 py-1.5 flex items-center gap-1">
              {Object.entries(REACTIONS).map(([key, emoji]) => (
                <button
                  key={key}
                  onClick={() => likePost.mutate({ postId: post.id, reaction: key })}
          ...
          <button
            onClick={() => likePost.mutate({ postId: post.id, reaction: currentReaction })}
            className={`w-full flex items-center justify-center h-11 gap-2 text-sm font-normal rounded-md border-none transition-all duration-200 ${
              isLiked ? "text-(--color5) bg-(--color9)" : "text-(--color6) bg-transparent hover:bg-(--color9)"
            }`}
          >
            {isLiked ? (
              <span className="text-lg">{REACTIONS[currentReaction] || "👍"}</span>
            ) : (
        ...
        <button
          ...
          className={`flex-1 flex items-center justify-center h-11 gap-2 text-sm font-normal rounded-md border-none mr-1 transition-all duration-200 ${
            showComments ? "text-(--color5) bg-(--color9)" : "text-(--color6) bg-transparent hover:bg-(--color9)"
          }`}
        >
  ...
  if (isError) {
    return (
      <div className="bg-(--bg2) rounded-md p-6 text-center mb-4">
        <p className="text-(--color7)">Failed to load posts. Please try again.</p>
      </div>
    );
  }
  ...
  if (posts.length === 0) {
    return (
      <div className="bg-(--bg2) rounded-md p-8 text-center mb-4">
        <p className="text-(--color7) text-base">No posts yet. Be the first to share something!</p>
      </div>
    );
  }
